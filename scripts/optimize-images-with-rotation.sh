...
# Image Optimization Script with Rotation Preservation
# Converts images to WebP, optimizes quality, and preserves orientation
...
echo -e "${BLUE}🖼️  Image Optimization with Rotation Preservation${NC}"
echo "=================================================="
...
get_orientation() {
    local file="$1"
    local orientation=$(identify -format "%[EXIF:Orientation]" "$file" 2>/dev/null || echo "")
    echo "$orientation"
}

# Function to optimize a single image with rotation preservation
optimize_image_with_rotation() {
    local input_file="$1"
    local output_file="$2"
    local original_size=$(get_file_size "$input_file")
    local orientation=$(get_orientation "$input_file")
    ...
    # Use ImageMagick to handle rotation and convert to WebP
    # This preserves the orientation by applying the rotation during conversion
    ...
    echo -e "  ${GREEN}✓${NC} Rotation: Preserved"
}
...
process_directory() {
    ...
    for file in "${image_files[@]}"; do
        ...
        # Optimize the image
        optimize_image_with_rotation "$file" "$output_file"
